...
resourcename="stockmarkettracker"  # lower case, only
resourcegroup="stockmarkettracker_rg"
...
# git clone https://github.com/apilogicserver/stockmarkettracker.git
...
az group create --name stockmarkettracker_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group stockmarkettracker_rg --sku S1 --is-linux
az container create --resource-group stockmarkettracker_rg --name stockmarkettracker --image apilogicserver/stockmarkettracker:latest --dns-name-label stockmarkettracker --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//stockmarkettracker.westus.azurecontainer.io:5656'
